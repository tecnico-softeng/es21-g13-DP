...
public class AnswersXmlExportVisitor implements Visitor {
    ...
    public void visitAnswerDetails(ItemCombinationAnswer answer) {
        ...
        if (answer.isAnswered()){
            Element itemContainerElement = new Element("items");
            itemContainerElement.setAttribute(QUESTION_KEY, String.valueOf(answer.getQuestionAnswer().getQuestion().getKey()));

            for (Item item: answer.getAnswerItems()) {
                Element itemElement = new Element("item");
                itemElement.setAttribute(QUESTION_KEY, String.valueOf(answer.getQuestionAnswer().getQuestion().getKey()));
                itemElement.setAttribute("itemSequence", String.valueOf(item.getSequence()));
                itemContainerElement.addContent(itemElement);
            }

            this.currentElement.addContent(itemContainerElement);
        }
    }
}
