...
public class QuestionsXmlImport {
    ...
    private QuestionDetailsDto importItemCombinationQuestion(Element questionElement){
        Set<ItemDto> items1 = new LinkedHashSet<>();
        Set<ItemDto> items2 = new LinkedHashSet<>();
        for (Element itemElement : questionElement.getChild("items").getChildren("item")) {
            ...
            Integer itemGroup = Integer.valueOf(itemElement.getAttributeValue("group"));
            String[] links = itemElement.getAttributeValue("links").split(" ");
            ItemDto itemDto = new ItemDto();
            itemDto.setContent(itemContent);
            itemDto.setSequence(itemSequence);
            for(String link: links)
                itemDto.addLink(Integer.valueOf(link));
        ...
        itemCombinationQuestionDto.setFirstGroup(items1);
        itemCombinationQuestionDto.setSecondGroup(items2);
    ...
    private QuestionDetailsDto importOpenAnswerQuestion(Element questionElement) {
        OpenQuestionDto questionDto = new OpenQuestionDto();
        questionDto.setAnswer(questionElement.getChild("answer").getAttributeValue("content"));
        return questionDto;
    }
