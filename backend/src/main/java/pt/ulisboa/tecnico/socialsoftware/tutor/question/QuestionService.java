...
public class QuestionService {
    ...
    @Retryable(
            ...
            backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.READ_COMMITTED)
    public ItemDto createItem(ItemDto item1) {
    ...
    @Retryable(
            ...
            backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.READ_COMMITTED)
    public void createLink(ItemDto itemOne, ItemDto itemTwo) {
        Item item1 = itemRepository
                .findById(itemOne.getId())
                .orElseThrow(() -> new TutorException(ITEM_NOT_FOUND, itemOne.getId()));
        Item item2 = itemRepository
                .findById(itemTwo.getId())
                .orElseThrow(() -> new TutorException(ITEM_NOT_FOUND, itemTwo.getId()));
        if (item1.getLinks().contains(item2)){
            throw new TutorException(LINK_ALREADY_EXISTS, item1.getId(), item2.getId());
        }
        item1.addLink(item2);
        item2.addLink(item1);
        item1.getQuestionDetails().getQuestion().setNumberOfCorrect(item1.getQuestionDetails().getQuestion().getNumberOfCorrect() + 1);
    }
    ...
    @Retryable(
            ...
            backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.READ_COMMITTED)
    public void removeLink(ItemDto item_1, ItemDto item_2) {
        Item item1 = itemRepository
                .findById(item_1.getId())
                .orElseThrow(() -> new TutorException(ITEM_NOT_FOUND, item_1.getId()));
        Item item2 = itemRepository
                .findById(item_2.getId())
                .orElseThrow(() -> new TutorException(ITEM_NOT_FOUND, item_2.getId()));
        if (!item1.getLinks().contains(item2)){
            throw new TutorException(LINK_DOES_NOT_EXIST, item1.getId(), item2.getId());
        }
        item1.deleteLink(item2);
        item2.deleteLink(item1);
        item1.getQuestionDetails().getQuestion().setNumberOfCorrect(item1.getQuestionDetails().getQuestion().getNumberOfCorrect() - 1);
    }
