...
class UpdateQuestionTest extends SpockTest {
    ...
    def "update an answer relevance"() {
        given: "a changed question"
        def questionDto = new QuestionDto(question)
        def multipleChoiceQuestionDto = new MultipleChoiceQuestionDto()
        questionDto.setQuestionDetailsDto(multipleChoiceQuestionDto)
        and: 'a the old wrong option'
        def newOptionKO = new OptionDto(optionKO)
        and: 'a new correct option with different relevance'
        def newOptionOK = new OptionDto()
        newOptionOK.setContent(OPTION_1_CONTENT)
        newOptionOK.setCorrect(true)
        newOptionOK.setRelevance(2)
    ...
    def "update question with two options true"() {
        ...
        and: 'a new correct option'
        def newOptionOK2 = new OptionDto()
        newOptionOK2.setContent(OPTION_1_CONTENT)
        newOptionOK2.setCorrect(true)
        and: 'add options to dto'
        def newOptions = new ArrayList<OptionDto>()
        newOptions.add(newOptionOK)
        newOptions.add(newOptionOK2)
        multipleChoiceQuestionDto.setOptions(newOptions)
        and: 'a count to load options to memory due to in memory database flaw'
        optionRepository.count();
    ...
    def "update an open answer question"() {
        given: "a open answer question"
        def openQuestion = new Question()
        openQuestion.setCourse(externalCourse)
        openQuestion.setKey(2)
        openQuestion.setTitle(QUESTION_2_TITLE)
        openQuestion.setContent(QUESTION_2_CONTENT)
        openQuestion.setStatus(Question.Status.AVAILABLE)
        openQuestion.setNumberOfAnswers(1)
        openQuestion.setNumberOfCorrect(1)
        def questionDetails = new OpenQuestion()
        questionDetails.setAnswer(OPEN_ANSWER_1)
        openQuestion.setQuestionDetails(questionDetails)
        questionDetailsRepository.save(questionDetails)
        questionRepository.save(openQuestion)

        and: "a questionDto with a changed correct answer"
        def questionDto = new QuestionDto(openQuestion)
        def openQuestionDetailsDto = new OpenQuestionDto()
        openQuestionDetailsDto.setAnswer(OPEN_ANSWER_2)
        questionDto.setQuestionDetailsDto(openQuestionDetailsDto)

        when:
        questionService.updateQuestion(openQuestion.getId(), questionDto)

        then: "the question has modified its answer"
        def questionResult = questionRepository.findAll().get(1).getQuestionDetails()
        questionResult.getAnswer() == OPEN_ANSWER_2
        def detailsResult = questionDetailsRepository.findAll().get(1)
        detailsResult.getAnswer() == OPEN_ANSWER_2
    }
