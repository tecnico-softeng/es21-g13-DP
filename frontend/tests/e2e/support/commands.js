...
Cypress.Commands.add('viewSolvedQuiz',
  (content1, content2) => {
    cy.contains(content1);
...
Cypress.Commands.add('solveQuizz', (quizTitle, numberOfQuizQuestions) => {
  ...
  for (let i = 1; i < numberOfQuizQuestions; i++) {
    ...
    cy.get('[data-cy="optionList"]').children().eq(2).click();
    cy.get('[data-cy="nextQuestionButton"]').click();
  }
...
Cypress.Commands.add('createICQuestion',
    (title, question, item1, item2) => {
        cy.get('[data-cy="managementMenuButton"]').click();
        cy.get('[data-cy="questionsTeacherMenuButton"]').click();

        //creates question1
        cy.get('[data-cy="newQuestionButton"]').click();
        cy.get('[data-cy="questionTitleTextArea"]').type(title, {force: true});
        cy.get('[data-cy="questionQuestionTextArea"]').type(question, {force: true});
        ...
        cy.get('[data-cy="questionFirstGroupItemsInput"')
            ...
            .each(($el, index, $list) => {
                cy.get($el).within(($ls) => {
                    cy.get(`[data-cy="Item1${index + 1}"]`).type(item1);
                });
            });
        cy.get('[data-cy="questionSecondGroupItemsInput"')
            ...
            .each(($el, index, $list) => {
                cy.get($el).within(($ls) => {
                    cy.get(`[data-cy="Item2${index + 1}"]`).type(item2);
                });
            });
        ...
        cy.get(".v-menu__content").contains(item2).click();
...
Cypress.Commands.add('solveICQuizz', (quizTitle, numberOfQuizQuestions, link) => {
    cy.get('[data-cy="quizzesStudentMenuButton"]').click();
    cy.contains('Available').click();
    ...
    for (let i = 1; i <= numberOfQuizQuestions; i++) {
        cy.get(`[data-cy="addLinksItemCombination1"]`).parent().click();
        cy.get(".v-menu__content").contains(link).click();
        cy.route('POST', '/courses/*/questions/').as('postQuestion');
        cy.get('button').contains('Store Links').click();
    }
